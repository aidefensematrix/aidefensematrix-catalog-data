...
1. **Every detail needs a trusted source.** Each detail carries a `source` with a URL and an `accessed` date. The source must actually support the specific detail. Cite the exact page, not the homepage. On `matrix_coverage` and `compliance_attestations` sources a `quote` is **required**: the verbatim supporting line from that page, in the page's own words (10 to 300 characters). It shows *why* the cell or attestation is claimed, gives a reviewer the starting point for confirming the mapping, and is periodically re-checked against the live page. **What the quote has to carry:** one verbatim line evidencing the row's core claim — the asset, and the function named in `primary_cell` where the row holds it. A row often lists several functions, and a single sentence rarely covers all of them; the `note` states the full claim, and the cited page as a whole is the evidence. So a `runtime-ai-data [protect, detect]` row is well-sourced when its quote shows the blocking or the monitoring, and it is not an error for the other function's evidence to live elsewhere on the same page. What is *not* acceptable is a quote that evidences no claimed function at all, or a function the cited page never substantiates anywhere — drop the function in that case, or cite a page that supports it. When the cited page exposes no quotable text (a bot wall, a fully JS-rendered page, or a PDF), state a `quote_unavailable` waiver with the reason instead, for example `quote_unavailable: "Bot wall (HTTP 403) on trust center, 2026-07"`. A source states its evidence or states why it cannot, never both.
...
- `agent`: An automated contributor verified this value against the cited page, backed by the repository's tool checks. A person who later confirms the value may set `reviewed`.
